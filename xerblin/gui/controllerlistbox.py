...
class SourceWrapper(object):
	...
	def __init__(self, source, widget, index=None):
		...
		initialing the drag operation, and index is the index of the item
		in the widget's model object (which is presumably a ListModel
...
class DraggyListbox(Listbox):
	...
	def startDrag(self, event):
		...
		if i >= 0:
			...
			dnd_start(SourceWrapper(self.stack[i], self, i), event)
		return "break"
...
class ControllerListbox(DraggyListbox):
	...
	def dnd_motion(self, source, event):
		index = self.nearest(event.y_root - self.winfo_rooty())
	...
	def dnd_commit(self, source, event):
		...
		try:
			...
			self.stack.insert(index, source.source)
